...
import time # takip algoritmalaırnın çalışma üsrelerini karşılaştırmak için
import pandas as pd

# takip algoritmalaırnın opencv içindeki adları
# 7 tane temel takip algoritma var
...
tracker_name = "mil" # içlerinden birini seçiyoruz
tracker = OPENCV_OBJECT_TRACKERS[tracker_name]()
print("Tracker:", tracker_name)

gt = pd.read_csv("gt_new.txt") # geçen ders oluşturduğumuz gt elde etme

video_path = "MOT17-13-SDP.mp4" # video path
cap = cv2.VideoCapture(video_path) # video okuma
...
track_list = []
start_time = time.time() # burası sayaç burada zaman başlıyor. ileride durdurup çalışması için geçen süreye bakıcaz
...
while True:
    ...
    if ret:
        ...
        (H, W) = frame.shape[:2] # 3. index channel.
        ...
        if key == ord("t"):
            initBB = cv2.selectROI("Frame", frame, fromCenter = False) # takip eidlen yerden bir yer seçmek için 
...
# araç takip başarısını degerlendirme
